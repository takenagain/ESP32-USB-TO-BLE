...
usb_pins_config_t USB_Pins_Config = {
    -1, -1, // Keyboard DP and DM
    18, 17, // Mouse DP and DM
    -1, -1, // Disabled
    -1, -1  // Disabled
};

bool USB_SOFT_HOST::init(printcb_t onDataCB, ontick_t onTickCB)
{
  USB_SOFT_HOST::setISRAllocFlag(0);
  printf("Attaching onDataCB\n");
  setPrintCb(onDataCB);
  printf("Attaching onUSBMessageDecode\n");
  setUSBMessageCb(onUSBMessageDecode);
  printf("Attaching onTickCB\n");
  USB_SOFT_HOST::setTaskTicker(onTickCB);
  if (_init())
  {
    printf("Attaching Timer Task\n");
    ...
    printf("USB Soft Host Group timer task is now running on core #%d with priority %d\n", core, priority);
    return true;
  }
  return false;
}
...
bool USB_SOFT_HOST::_init()
{
  if (inited)
    return false;

#if !defined USE_NATIVE_GROUP_TIMERS
  printf("Using custom group timer\n");
  ...
  printf("Seleting SCL (blink) Pin #%d\n", blink_gpio);

  gpio_pad_select_gpio(blink_gpio);
  gpio_set_direction(blink_gpio, GPIO_MODE_OUTPUT);

  printf("Creating timer config\n");
